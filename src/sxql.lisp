...
(defgeneric to-key-conditions (op table)
  ...
  (:method ((op conjunctive-op) table)
    (let ((hash-key (attr-name (table-hash-key table)))
          (range-keys (mapcar #'attr-name (table-range-keys table)))
          (using-range-key-p)
          (rest))
      (values (mapcan #'(lambda (item)
                          (if (equal (sql-op-name item) "=")
                              (if (equal (car (op-keys item table)) hash-key)
                                  (to-key-conditions item table)
                                  (if (and (not using-range-key-p)
                                           (find (car (op-keys item table)) range-keys :test #'equal))
